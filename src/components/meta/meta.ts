...
class Meta{
    ...
    constructor(){
      //BELOW PAGES HELPS REDIRECT THE PAGE TO THE ERROR_PAGE IF THE PAGE DOES NOT EXIST
        ...
        Meta.domain=process.env.NODE_ENV !== "production" ? process.env.NEXTAUTH_URL as string : "http://localhost:3000";
        Meta.url=new URL(Meta.domain);
    ...
    static metaHome(item:{baseUrl}):Metadata{
      const {baseUrl}=item;
        return {
    ...
    static async genSitemapArray():Promise<MetadataRoute.Sitemap>{
      ...
      try {
        arr=[
          {url:`${Meta.domain}`,lastModified:new Date(),changeFrequency:'weekly',priority:1},
          {url:`${Meta.domain}/blogs`,lastModified:new Date(),changeFrequency:'daily',priority:1},
          {url:`${Meta.domain}/posts`,lastModified:new Date(),changeFrequency:'daily',priority:1},
          {url:`${Meta.domain}/editor`,lastModified:new Date(),changeFrequency:'weekly',priority:1},
          {url:`${Meta.domain}/policy`,lastModified:new Date(),changeFrequency:'monthly',priority:1},
          {url:`${Meta.domain}/register`,lastModified:new Date(),changeFrequency:'yearly',priority:1},
          {url:`${Meta.domain}/termsOfService`,lastModified:new Date(),changeFrequency:'yearly',priority:1},
          {url:`${Meta.domain}/signin`,lastModified:new Date(),changeFrequency:'yearly',priority:1},
          {url:`${Meta.domain}/chart`,lastModified:new Date(),changeFrequency:'monthly',priority:1},
        ];
        ...
        if(getBlogs && getBlogs.length>0){
          getBlogs.map(blog=>{
            arr.push({url:`${Meta.domain}/blog/${blog.id}`,lastModified:new Date(),changeFrequency:'always',priority:1})
          });
        }
      } catch (error) {
