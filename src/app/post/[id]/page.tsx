...
export async function genMetadata(item: { id: number, parent: ResolvingMetadata }): Promise<Metadata> {
    ...
    const img = post?.image || "https://www.ablogroom.com/images/posts.png";
    const image = imageLoader({ src: img, width: 300, quality: 75 });
    const title = post?.title as string || "Ablogroom blogs";
    const keywds = post?.content ? await genKeywords({ content: post.content, title }) : [];
